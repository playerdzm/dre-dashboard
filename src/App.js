...
const firebaseConfig = {
  ...
  storageBucket: "dre-dashboard-667b0.firebasestorage.app",
  ...
  <div className="mb-4">
    ...
    <div className="mt-1 relative rounded-md shadow-sm">
      ...
      <input type={type} name={name} id={name} value={value === null || value === undefined ? '' : value} onChange={onChange} placeholder={placeholder || (type === 'number' ? "0" : "")}
        className={`block w-full pr-10 sm:text-sm border-gray-300 rounded-md focus:ring-[${V4_RED}] focus:border-[${V4_RED}] ${unit === 'R$' ? 'pl-10' : 'pl-3'} ${disabled ? 'bg-gray-100 cursor-not-allowed' : ''}`}
  ...
  return (
    <div className="mb-2 p-3 bg-gray-50 rounded-md shadow-sm">
      <div className="flex justify-between items-center">
        ...
        <span className={`text-sm font-semibold ${typeof value === 'number' && value < 0 ? 'text-red-600' : `text-[${V4_RED}]`}`}>{displayValue}</span>
      </div>
    </div>);
};
...
const DashboardCard = ({ title, value, icon, isPercentage = false, isCurrency = true, info }) => {
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg min-h-[100px] sm:min-h-[120px] flex flex-col justify-between">
      <div>
        <div className="flex items-center mb-1"><div className={`text-[${V4_RED}]`}>{icon}</div><h3 className="text-sm sm:text-md font-semibold text-gray-700 ml-2">{title}</h3></div>
        {info && <p className="text-xs text-gray-500 mb-1">{info}</p>}
      </div>
      <p className={`text-xl sm:text-2xl font-bold ${typeof value === 'number' && value < 0 ? 'text-red-500' : `text-[${V4_RED}]`}`}>{displayValue}</p>
    </div>);
};
...
const Button = ({ onClick, children, variant = 'primary', className = '', type = 'button', disabled = false, id }) => {
  ...
  const variantStyles = {
    primary: `bg-[${V4_RED}] text-white hover:bg-red-700 focus:ring-[${V4_RED}]`,
    secondary: "bg-gray-200 text-gray-700 hover:bg-gray-300 focus:ring-gray-400",
    danger: "bg-red-500 text-white hover:bg-red-600 focus:ring-red-400",
    outline: `border border-[${V4_RED}] text-[${V4_RED}] hover:bg-red-50 focus:ring-[${V4_RED}]`
  };
...
const App = () => {
  ...
  <div className="flex h-screen bg-gray-100 font-sans"> 
    ...
    <aside className={`bg-white text-gray-700 w-64 space-y-6 py-7 px-2 absolute inset-y-0 left-0 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition-transform duration-200 ease-in-out z-30 shadow-lg md:shadow-none flex flex-col`}> 
      <div className="px-4 mb-2"> <a href="#" onClick={(e) => {e.preventDefault(); setCurrentPage('dashboard'); setSidebarOpen(false);}} className={`text-2xl font-extrabold text-[${V4_RED}]`}>{V4_TEXT_LOGO}</a> <p className="text-xs text-gray-500">Dashboard Financeiro</p> </div> 
      <nav className="flex-grow"> {navigationItems.map(item => ( <a key={item.id} href="#" onClick={(e) => {e.preventDefault(); setCurrentPage(item.id); setSidebarOpen(false);}} className={`flex items-center space-x-3 py-2.5 px-4 rounded-md text-sm hover:bg-red-50 hover:text-[${V4_RED}] transition-colors ${currentPage === item.id ? `bg-[${V4_RED}] text-white font-semibold shadow-md` : 'text-gray-600'}`}> {item.icon}<span>{item.label}</span> </a>))} </nav> 
      ...
      <header className="bg-white shadow-sm p-4"> 
        <div className="flex items-center justify-between"> 
          <div className="flex items-center"> 
            ...
            {(currentPage === 'dashboard' || currentPage === 'dre' || currentPage === 'assumptions') && selectedClientId && <Edit3 size={18} className="ml-2 text-gray-400 hover:text-[${V4_RED}] cursor-pointer flex-shrink-0" title="Mudar Cliente" onClick={() => setCurrentPage('clients')}/> } 
          </div> 
